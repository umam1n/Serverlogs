...
def process_check_in(request, log_id):
    ...
    if request.method == 'POST':
        ...
        if not is_user_in_range(user_lat, user_lon, site_location.latitude, site_location.longitude):
        ...
        if verified:
            ...
            # If face verification fails, redirect back to the check-in page to show the error.
            return redirect('process_check_in', log_id=log.id)
            
    # This block handles the initial GET request to show the page
    return render(request, 'logs/process_check_in.html', {'log': log})
